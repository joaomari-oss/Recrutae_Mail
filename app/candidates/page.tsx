              ...
              <div className="p-4 rounded-xl bg-brand-charcoal border border-white/5 animate-fade-up">
                <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto">
                  {parsedCandidates.map((c) => (
                    <span key={c.id} className="inline-flex items-center gap-1 bg-brand-dark border border-white/8 rounded-full px-2.5 py-1 text-xs text-brand-muted">
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {(showAllTable ? parsedCandidates : parsedCandidates.slice(0, 8)).map((c, i) => (
                    <tr key={c.id} className="hover:bg-white/2 transition-colors animate-fade-up opacity-0" style={{ animationDelay: `${i * 30}ms`, animationFillMode: 'forwards' }}>
              ...
              {parsedCandidates.length > 8 && (
                <div className="px-4 py-3 text-center border-t border-white/5 bg-brand-charcoal/50">
                  <button onClick={() => setShowAllTable(!showAllTable)} className="flex items-center gap-1.5 mx-auto text-xs text-brand-coral hover:text-brand-orange transition-colors">
                    {showAllTable ? (<>Mostrar menos <ChevronUp className="h-3 w-3" /></>) : (<>Ver todos os {parsedCandidates.length} contatos <ChevronDown className="h-3 w-3" /></>)}
                  </button>
                </div>
              )}
            </div>
